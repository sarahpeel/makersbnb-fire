...
class ListingRepository:
    ...
    def all(self):
        ...
        listings = []
        for row in rows:
            item = Listing(row['id'], row['name'], row['description'], row['location'], row['price'], row['occupied'])
    ...
    def find(self, listing_id):
        ...
        return Listing(rows[0]['id'], rows[0]['name'], rows[0]['description'], rows[0]['location'], rows[0]['price'], rows[0]['occupied'])
