...
def test_find_booking(db_connection):
    ...
    assert found == [Booking(5, 3, 1, x, y, 'requested')]
...
def test_find_booking_by_listing_user(db_connection):
    ...
    assert result == [Listing(3, 'Windsor Castle', 'Really old', 'Windsor', 1150, 2, [Booking(5, 3, 1, x, y, 'requested')])]
...
def test_find_multiple_bookings_by_multiple_listings_user(db_connection):
    ...
    assert result == [Listing(1, 'Blackpool Tower', 'Really high', 'Blackpool', 25, 1, [Booking(1, 1, 2, listing1_start, listing1_end, 'requested'), Booking(2, 1, 3, listing2_start, listing2_end, 'requested')]), Listing(2, 'London Eye', 'Really round', 'London', 150, 1, [Booking(3, 2, 3, listing3_start, listing3_end, 'confirmed'), Booking(4, 2, 3, listing4_start, listing4_end, 'confirmed')])]
...
def test_create_booking(db_connection):
    ...
    assert result == [Booking(6, 4, 1, x, y, 'requested')]
...
def test_change_status_from_requested_to_confirmed(db_connection):
    ...
    assert result == [Booking(1, 1, 2, x, y, 'confirmed')]
...
def test_change_status_from_requested_to_denied(db_connection):
    ...
    x = datetime.date(2023, 9, 10)
    y = datetime.date(2023, 9, 12)
    assert result == [Booking(1, 1, 2, x, y, 'denied')]